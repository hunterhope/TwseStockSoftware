...
public class StockDayInfoRepository {
    ...
    private List<StockDayInfo> noTableAction(String stocdId,Consumer<String> updateMsgAction) throws StockIdNetNoDataException {
        ...
        try {
            ...
            tdss.addListener(si);
            //上網抓取
            tdss.crawl(stocdId, LocalDate.now(), 2);//抓2個月是為了確保個月的第1天碰到交易日貨不是交易日,而沒資料,其實有錢一個月資料
            try {
                return tdqs.selectAllDayInfo(stocdId, StockDayInfo.class);
            ...
            throw new StockIdNetNoDataException(stocdId);
        } finally {
            //移除監聽器
